...
init_func() {
    "$PWD"/build/jkd keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/jkd init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/jkd add-genesis-account \
    "$("$PWD"/build/jkd keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000ajk,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/jkd gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/jkd collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/jkd validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/jkd start --pruning=nothing --rpc.unsafe \
    ...
    JK_PID=$!
    echo "started evmos node, pid=$JK_PID"
    # add PID to array
    arr+=("$JK_PID")
...
stop_func() {
    JK_PID=$i
    echo "shutting down node, pid=$JK_PID ..."
