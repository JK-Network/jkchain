...
Details of non-native currencies (ie. not `ajk`) can be queried with the following CLI command:
...
Developers can query account balances of `ajk` using the [`eth_getBalance`](../json-rpc/endpoints.md#ethgetbalance) JSON-RPC method in conjunction with [`curl`](https://curl.se/):
