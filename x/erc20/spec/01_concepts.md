...
A native Cosmos Coin corresponds to an `sdk.Coin` that is native to the bank module. It can be either the native staking/gas denomination (eg: JK, ATOM, etc) or an IBC fungible token voucher (i.e with denom format of `ibc/{hash}`).
...
More sophisticated malicious implementations might also inherit code from customized ERC20 contracts that include malicous behaviour. For an overview of more extensive examples, plese review the x/erc20 audit, section `IF-JK-06: IERC20 Contracts may execute arbitrary code`.
