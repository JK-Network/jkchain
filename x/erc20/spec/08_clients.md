...
Find below a list of  `jkd` commands added with the  `x/erc20` module. You can obtain the full list by using the `jkd -h` command. A CLI command can look like this:

```bash
jkd query erc20 params
```
...
```bash
jkd tx gov submit-proposal register-coin METADATA_FILE [flags]
```
...
```bash
jkd tx gov submit-proposal register-erc20 ERC20_ADDRESS... [flags]
```
...
```bash
jkd tx gov submit-proposal toggle-token-conversion TOKEN [flags]
```
