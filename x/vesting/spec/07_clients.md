...
Find below a list of `jkd` commands added with the `x/vesting` module. You can obtain the full list by using the `jkd -h` command.
...
```go
jkd add-genesis-account ADDRESS_OR_KEY_NAME COIN... [flags]
```
...
```go
jkd query vesting balances ADDRESS [flags]
```
...
```go
jkd tx vesting create-clawback-vesting-account TO_ADDRESS [flags]
```
...
```go
jkd tx vesting clawback ADDRESS [flags]
```
