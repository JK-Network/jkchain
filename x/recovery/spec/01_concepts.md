...
2. Osmosis/Cosmos Hub account with claims record sent IBC transfer to Evmos `secp256k1` receiver address
    ...
    **Consequences**
    ...
    - IBC vouchers  from IBC transfer got stuck in the receiver’s balance
    - IBC Transfer Action was claimed and the JK rewards were transferred to the receiver’s Evmos `secp256k1` account, resulting in stuck JK tokens.
...
1. User attempts to recover `1000ajk` that are stuck on the Evmos chain.
...
3. Evmos receives the transaction, and goes through the IBC stack:
    ...
    3. **Recovery Middleware**: since `sender=receiver` -> recover user balance (`1000ajk` and `100uosmo`) by sending an IBC transfer from `receiver` to the `sender` on the Osmosis chain.
4. User receives `100uosmo` and `1000ajk` (IBC voucher) on Osmosis.
